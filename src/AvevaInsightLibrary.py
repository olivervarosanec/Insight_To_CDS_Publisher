...
class Aveva_Insight:
    ...
    def api_call(self, method, url, params=None, data=None, process_func=None):
        ...
        while True:
            ...
            if '@odata.nextLink' in response.json():
                ...
                if self.debug:
                    print(f"page: {counter}", flush=True, end="\r")
            else:
                break
    ...
    def get_Insight_Data(self, tagnames, starttime=None, endtime=None, relative_days=None, RetrievalMode=None, Resolution=None, InterpolationType=None):
        ...
        # Yield each chunk of data from api_call
        for chunk in self.api_call("get", api_url, params=params, process_func=lambda df: df.sort_values('DateTime', ascending=True)):
            if len(chunk) > 0:
                chunk['DateTime'] = pd.to_datetime(chunk['DateTime'], format='mixed')
            yield chunk

    
    def check_tag_data_exists(self, tagname):
        ...
        params = {
            "TagFilter": f"endswith(FQN, '{tagname}')",
            "$filter": f"DateTime ge {starttime} and DateTime le {endtime}",
            "RetrievalMode": "Delta"
        }

        # Make a single API call without paging
        ...
        data_values = data.get('value', [])
        data_length = len(data_values)
        if data_length <= 1:
            if data_length == 0 or data_values[0].get('Value') is None:
                return False
            else:
                return True
        else:
    ...
    def get_Expression_Data(self, expression, starttime=None, endtime=None, relative_days=None, RetrievalMode=None, Resolution=None):
        ...
        # Yield each chunk of data from api_call
        for chunk in self.api_call("post", api_url, data=payload, process_func=lambda df: df.sort_values('DateTime', ascending=True)):
    ...
    def get_Tag_List(self, tagnames=None):
        # Verify the input
        if tagnames is not None and not isinstance(tagnames, str):
            raise ValueError("tagnames must be a string")

        api_url = self.base_url + self.tags_path

        payload = {
            "Query": {
                "Location": f"startswith(FQN, '{tagnames}')" if tagnames else "",
                "OrderByClause": "Location, FQN",
                "Select": "FQN,Source,Description,EngUnit,Location, TagType"
            }
        }

        df_list = []
        for chunk in self.api_call("post", api_url, data=payload):
